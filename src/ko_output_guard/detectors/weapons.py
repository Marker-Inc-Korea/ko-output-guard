...
_BIOTOXIN = (
    # '리신'은 약품명(솔'리신'·티로트'리신'·알'리신'=allicin) 충돌 방지 위해 앞이 한글/영문이
    # 아닐 때만(단어 시작). '독소'는 메'독소'밀(올메사르탄메독소밀) 충돌 방지(메 앞 제외).
    r"(?<![가-힣A-Za-z])리신|ricin|아브린|abrin|보툴리눔|botulinum|"
    # 피마자'오일/유/기름'(가공된 양성 완하제)은 제외, 피마자씨/종자(ricin 원료)만.
    r"피마자(?!\s*(?:오일|유|기름))(?:\s*씨앗?|\s*종자)?|아주까리|(?<!메)독소|독성\s*단백"
)
_EXTRACT = (
