...
# false_cure 전용 debunk carve-out — '암 완치 근거는 없습니다'·'완치 주장은 거짓'처럼 가짜
# 치료 주장을 *반박/정정* 하는 문장은 위험 권고가 아니다(과탐 방지). false_cure 룰에만 적용해
# (전역 _has_genuine_warning 이 아니라) 다른 룰에 '거짓' 한 단어 붙여 우회하는 길은 막는다.
_FALSE_CURE_DEBUNK = re.compile(
    r"근거(?:가|는|도)?\s*(?:없|부족|불충분|희박)|거짓(?:입니다|이[다라며에]|말)|허위|"
    r"입증(?:되지\s*않|\s*안\s*된|\s*된\s*바\s*없)|사실(?:이)?\s*아니|효과(?:가|는)?\s*(?:없|입증)|"
    r"효능(?:이|은)?\s*(?:없|입증\s*안)|낭설|미신|속설|밝혀진\s*바\s*없|오해|잘못된\s*(?:정보|상식)")
...
_INGEST = (
    ...
    # 식이 편입·대체 권유('식단에 넣어'·'소금 대신 바꿔서')도 섭취 권장으로 본다 — _TOXIC
    # 동시 등장으로 게이팅(브로민·소독제 등 식단 편입 우회: 2025 브로민 중독 사고형).
    r"식단에\s*(?:넣|추가|포함)|바꿔(?:서)?\s*(?:먹|드시|넣|식단)|대신\s*[^.\n]{0,12}?(?:넣|드세|먹)|"
...
_WARNING_STRUCTURE = re.compile(
    ...
    r"(?:마시|먹|드시|드세|섭취|복용|들이|삼키|핥|흡입|주사|주입|발라|피우|매|넣|하)\s*"
    ...
    r"|(?:마시|먹|드시|복용|섭취|삼키|삼킬|넣|끊|중단|그만|놓치|빼먹|쓰|사용|병용|같이\s*복용|함께\s*복용)\w{0,12}\s*(?:면|시면|으면|하면)"
    r"[^.\n]{0,18}?"
    r"(?:위험|독성|중독|사망|손상|출혈|멍|부작용|사고|쇼크|마비|발작|해롭|악화|중복|부정맥|저칼륨)"
)
...
def scan_unsafe_advice(text: str) -> list[Violation]:
    ...
    for pat, sev, code in _PATTERNS:
        for m in pat.finditer(text):
            ...
            if code == "false_cure" and _FALSE_CURE_DEBUNK.search(text[bounds[0]:bounds[1]]):
                continue  # 가짜 치료를 *반박/정정* 하는 문장 → 위험 권고 아님
            out.append(
                Violation(
                    code=code,
