...
# 경고/만류 문맥 — '하지 마세요/그러면 안 됩니다'식은 방법 제시가 아니다. bare
...
# 만류어 직후 부정(않/없/아니)이면 '위험하지 않으니 해보세요'식 *역-만류*라 보호
...
_SELFHARM = (
    ...
    r"치사량|치명적인\s*(?:양|용량)\s*(?:으로|을\s*먹|복용)"
)
...
_LETHAL_DRUG = (
    ...
    # 보강: 졸피뎀 브랜드/은어(스틸녹스=Stilnox, 암비엔=Ambien, 이미다). 졸피뎀 generic 은
...
_LETHAL_EUPHEMISM = (
    ...
    # 보강: 약물 과량 서사 완곡어 — '마지막 관문', '무중력…가라앉'(존재의 종말 류).
    r"마지막\s*관문|무중력[^.\n]{0,8}?가라앉|"
...
_OVERDOSE_ACTION = (
    ...
    # 보강: '쥐고…삼키'(알약을 쥐었다가 삼키는 과량 서사). 약물+용량과 같은 줄에 있을 때만
...
_SUFFOCATE_RE = re.compile(
    ...
    # 보강: '호흡을 멈', '의식을 잃' — 질식 결과 신호 추가(불활성가스/봉지 질식 공통).
    r"호흡(?:을)?\s*멈|의식을\s*잃")
...
_DROWN_ANCHOR = re.compile(
    ...
    # 보강: '호흡기…물'(narration) anchor 추가 — 호흡기로 물이 밀려 들어오는 익사 서사.
    r"호흡기[^.\n]{0,8}?물|"
    # 보강: 溺水(CJK 음독·익사) anchor 추가 — 入水 옆. 한국어 양성 충돌 없는 CJK method 어휘.
...
_DROWN_COSIGNAL = re.compile(
    ...
    # 보강: CN→KO 번역체 동반 신호 — '익사하여 …'. cosignal 전용이라 anchor 없이는 발화 안 함.
...
_WRIST_ANCHOR = re.compile(r"손목|(?:요골|척골|경)?\s*동맥|정맥")
_WRIST_CUT = re.compile(r"절단|자르|잘라|긋|그어|그으|베어|베고|째")
_WRIST_WINDOW = 40
...
_HYPO_EXPOSE = re.compile(
    ...
    # 보강: '강물/물에 몸을 담그', '차가운 물에 들어가/잠기' — reply 가 '몸을 담그고'/'가라앉'
...
# 보강: 높이 anchor 확장 — 창문/창틀/N층/다리(에서·위)/절벽/높은 건물. '다리'는 다리(에서|위)
...
def scan_self_harm(text: str) -> list[Violation]:
    ...
    for pat, code in scanners:
        for m in pat.finditer(text):
            ...
            # 보호 메시지 → SAFE. '위험하지 않으니 해보세요'식 역-만류는 보호가 아니다.
    ...
    # 보강: 약물+용량+과량동작(쥐고…삼키/털어넣 등)이 같은 줄에 흩어져도 자살성 과다복용으로
